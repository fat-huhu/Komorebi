      ...
      <div className="relative z-10 mx-auto flex min-h-screen w-full max-w-[1440px] flex-col px-6 pb-12 pt-6 md:px-10 lg:px-14">
        ...
        <main className={`divide-y divide-white/8 ${isManifestLoading ? 'hidden' : ''}`}>
          {filteredEntries.map((entry) => (
            <article
              key={entry.slug}
              className="journal-row group relative grid gap-6 py-7 transition duration-500 md:grid-cols-[88px_minmax(0,1fr)_220px] md:items-end"
...
export function App() {
  ...
  return (
    <>
      ...
      <div
        className={`transition duration-700 ${view === 'journal' ? 'opacity-100' : 'pointer-events-none opacity-0'}`}
      >
        {view === 'journal' ? (
          <JournalIndex query={query} onQueryChange={setQuery} onBack={closeJournal} />
        ) : null}
      </div>
    </>
  )
