...
import { ArrowUpRight, Music, BookOpen, Boxes } from 'lucide-react'
...
function ParticleBurst({ color, trigger }: { color: string; trigger: boolean }) {
  const particles: Particle[] = Array.from({ length: 12 }, (_, i) => ({
    ...
    x: 50,
    y: 50,
    vx: Math.cos((i / 12) * Math.PI * 2) * (34 + (i % 4) * 11),
    vy: Math.sin((i / 12) * Math.PI * 2) * (34 + ((i + 2) % 4) * 11),
    color,
  }))
...
export function HomeExperience({
  ...
  return (
    <div className="relative w-full bg-[#050505] text-[#eaeaea] selection:bg-orange-500 selection:text-black">
      {/* ── Fixed nav & HUD ─────────────────────────────────────────────── */}
      <div className="fixed right-6 top-6 z-[60] flex flex-col items-end gap-3">
        <MagneticButton
          onClick={onEnterJournal}
          className="flex items-center gap-3 border border-white/12 bg-black/45 px-4 py-3 text-[11px] font-medium uppercase tracking-[0.38em] text-white/88 backdrop-blur-xl transition duration-500 hover:-translate-y-0.5 hover:border-orange-400/60 hover:bg-orange-500/10 hover:text-white"
        >
        ...
        <MagneticButton
          ...
          className="flex items-center gap-3 border border-white/12 bg-black/45 px-4 py-3 text-[11px] font-medium uppercase tracking-[0.38em] text-white/88 backdrop-blur-xl transition duration-500 hover:-translate-y-0.5 hover:border-cyan-300/60 hover:bg-cyan-300/10 hover:text-white"
        >
          <Boxes className="h-4 w-4 text-cyan-300" strokeWidth={1.7} />
          Stack Radar
          <ArrowUpRight className="h-4 w-4" strokeWidth={1.7} />
        </MagneticButton>
      </div>
          ...
          <div style={{ height: '200vh' }} className="relative overflow-hidden bg-black">
            <div className="sticky top-0 flex h-screen flex-col items-center justify-center">
              ...
              >
                ...
                <button
                  onClick={onEnterResources}
                  className="group relative overflow-hidden border border-white/15 bg-transparent px-8 py-4 font-bold tracking-[0.28em] text-white transition-colors duration-300 hover:border-cyan-300"
                >
                  <span className="relative z-10 inline-flex items-center gap-3">
                    Stack Radar
                    <Boxes className="h-4 w-4" strokeWidth={2} />
                  </span>
                  <div className="absolute inset-0 origin-left scale-x-0 bg-cyan-300/15 transition-transform duration-300 ease-out group-hover:scale-x-100" />
                </button>
                <button
